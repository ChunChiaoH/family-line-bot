...
- **記憶固化 job**（2026-07-15 討論）：現有記憶只從被觸發的對話形成，未觸發訊息中的
  事實進不了 KB。輕量版：使用者不定期叫 agent 讀 Firestore 近期訊息、對照 memory files
  出 diff、人工確認後寫入。自動版（驗證有價值後）：Cloud Scheduler 週跑 + Batches API 半價；
  原則：新增可自動、刪改必留人工確認。
