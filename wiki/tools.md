...
- **記憶固化 job**（2026-07-15 討論，2026-07-16 隨 D10 擴充）：現有記憶只從被觸發的
  ...
  Cloud Scheduler 週跑 + Batches API 半價；原則：新增可自動、刪改必留人工確認。
  **四個輸出**：(1) 事實更新 (2)「時刻」策展（事件/金句/情緒片段，寧缺勿濫）
...
- **語風習得（familect acquisition，2026-07-16 設計）**：bot 從通用人味漸進適應家庭
  ...
  **防胖核心＝定容重蒸**：語風檔 ≤300 字、範例固定 3 組（1 經典+2 輪替），每次從近四週
  ...
  config.py persona＝憲法層（job 無權碰），語風檔＝口音層（job 重寫）。護欄：學群體
  不學個人、適應速率宜慢。KB 結構配套：memory files 改人物 entity 制
  （people/某某.md + family.md + dates.md），4000 字爆掉時分層（prompt 常駐精華
  + memory view 讀深層——屆時反 view 指示要部分解禁）。
- **KB 快照版本化（2026-07-16 設計）**：固化 job 每次跑完，整份 KB（事實+時刻+側寫+語風）
  追加快照到 `chats/{id}/kb_snapshots/{日期}`，append-only、月量級幾 KB。快照間 diff
  即家庭編年史（梗的興衰、側寫的成長軌跡）；未來「去年這時候我們家」直接讀舊快照。
  live 檔可變、快照不可變——對 KB 做 git/wiki-log 對程式碼做的事。
